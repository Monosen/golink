...
export class ShortUrlService {
  ...
  async create(createShortUrlDto: CreateShortUrlDto, user: User) {
    try {
      ...
      throw new NotFoundException('Error creating short URL')
    }
  }
  ...
  async findOne(id: number, user: User) {
    try {
      ...
      console.error(`Error finding short URL with ID ${id}: ${error}`)
      throw new NotFoundException(`Short URL with ID ${id} not found`)
    }
  }
  ...
  async generateRandomCode(): Promise<{ randomCode: string }> {
    try {
      ...
      let randomCode = ''
      randomCode += uppercase[Math.floor(Math.random() * uppercase.length)]
      randomCode += lowercase[Math.floor(Math.random() * lowercase.length)]
